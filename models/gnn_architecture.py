...
# the GNN architectures will output the number of layers inside it in the forward() method
    ...
    def forward(self, data):
        ...
        for iter_num in range(self.layer_num):
            if torch.max(left_confidence).item() > 1e-7:
                data = Batch(x=self.next_x(x, new_x, left_confidence, self.decreasing_ratio), input_x = data.input_x)
                new_x = self.gnn_layer_module(data)

                global_feat = self.readout_module(Batch(x=new_x, input_x = data.input_x))
                current_confidence = self.confidence_module(global_feat)[batch]
                x = self.update_x(
